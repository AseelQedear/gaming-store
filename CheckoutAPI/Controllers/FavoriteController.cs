...
namespace CheckoutAPI.Controllers
{
    ...
    public class FavoriteController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddFavorite([FromBody] int deviceId)
        {
            ...
            return Ok(new { Message = "Device added to favorites." });
        }
        ...
        public async Task<IActionResult> RemoveFavorite(int deviceId)
        {
            ...
            var favorite = await _context.Favorites.FirstOrDefaultAsync(f => f.UserId == userId && f.DeviceId == deviceId);
            if (favorite == null) return NotFound("Favorite not found.");

            _context.Favorites.Remove(favorite);
            await _context.SaveChangesAsync();

            return Ok(new { Message = "Device removed from favorites." });
        }
